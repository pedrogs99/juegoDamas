...
void Tauler::setIfMensaje(bool Mensaje)
{
	T_Mensaje = Mensaje;
...
int Tauler::setMovimientos(int i)
{
...
{ /*Comprueba cuál es la casilla seleccionada y el turno, luego cual es la casilla destino,
	...
	if (Returner) //Si se quiere tener todos los movimientos, eliminar este if.
	{
		...
		Cola.add(MovSavior);//se guarda el movimiento en la cola si este es valido.
	}
...
int Tauler::acabaPartida()
{
	...
	for (int i = 0; i < MIDATAULER; i++)//comprueba cuantas ficchas quedan de cada color
	{
		for (int j = 0; j < MIDATAULER; j++)
		{
...
void Tauler::SaveGame(Screen & jocLib)
{
	...
	ClockWork(jocLib);//pinta la pantalla de carga
}
